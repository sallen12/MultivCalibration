...
In this vignette, we document the \pkg{MultivCalibration} \proglang{R} package, which allows multivariate rank histograms to be applied in practice. The following section introduces multivariate probabilistic calibration. Section \ref{sec:examples} then demonstrates how multivariate calibration can be assessed using \pkg{MultivCalibration}, before the package is used to reproduce the results in \cite{AllenEtAl2023}. A simulation study is presented in Section \ref{sec:simulation}, and these multivariate rank histograms are then used to evaluate probabilistic forecasts of gridded wind speed fields over Europe in Section \ref{sec:application}. A brief discussion of possible extensions is presented in Section \ref{sec:discussion}. For efficiency, results are presented for a subset of the data considered in \cite{AllenEtAl2023}, but the exact results therein can easily be generated by changing the parameters in Sections \ref{sec:simulation} and \ref{sec:application}.
...
\[
  \rho_{mn}(\bY) = \bar{Y} = \frac{1}{d} \sum_{j=1}^{d} Y_{j}.
\]
Similarly, the spread of the multivariate vector can be quantified using the sample variance,
\[
  \rho_{var}(\bY) = s_{\bY}^{2} = \frac{1}{d} \sum_{j=1}^{d} \left( Y_{j} - \bar{Y} \right) ^{2},
\]
while adaptations of the variogram can quantify the dependence between different dimensions,
\[
  \rho_{dep}(\bY; w) = -\frac{\sum_{i=1}^{d} \sum_{j=1}^{d} w_{i,j} | Y_{i} - Y_{j} | ^{2}}{s_{\bY}^{2}},
\]
for some non-negative weight matrix $w \in \R^{d \times d}$. The negative sign ensures that a larger value of $\rho_{dep}$ indicates a larger dependence, in keeping with $\rho_{mn}$ and $\rho_{var}$ above. This simplifies the interpretation of the resulting multivariate rank histograms.
...
The spatial variogram can also be used to generate a pre-rank function that quantifies the isotropy of the variogram. A variogram is said to be isotropic if it depends only on the distance between elements of the multivariate vector, and not on the direction between them. By introducing a pre-rank function that measures the isotropy of an empirical variogram, we can assess to what extent the multivariate ensemble forecasts reproduce the degree of (an)isotropy present in the observed outcomes.

Let $\mathbf{h} \in \{0, \dots, p - 1\} \times \{0, \dots, q - 1\}$ denote a multivariate lag, and let $\mathcal{I} = \{1, \dots, p\} \times \{1, \dots, q \}$ represent a set of grid points. We define the empirical variogram of $\bY \in \R^{p \times q}$ at multivariate lag $\mathbf{h}$ as
\[
...
\[
\rho_{iso}(\mathbf{x}; h) = -a(h) \left[ \gamma_{\mathbf{x}}((h, 0)) - \gamma_{\mathbf{x}}((0, h)) \right]^{2},
\]
where
\[
...
Some in-built pre-rank functions also require variable arguments. For example, to calculate the fraction of threshold exceedances, the user must specify a threshold \code{t}, a single numeric value. Similarly, the variogram pre-rank function requires a lag \code{h} or a weight matrix \code{w}. The lag can either be a single integer, or a vector of integers, in which case the variogram is calculated for all integers in the vector, and then summed to return a single value. The weight matrix must be symmetric and contain only non-negative values.

While \code{get_prerank()} assumes that \code{y} is a vector, pre-rank functions are also available when the observations and ensemble members are matrices. The \pkg{MultivCalibration} package additionally exports a function \code{get_prerank_gr()} that calculates pre-ranks corresponding to gridded objects.
\begin{Code}
...
In this case, the variogram pre-rank function requires an additional argument \code{h} that is a vector of length two. This denotes the spatial lag at which the variogram should be calculated. If \code{h} is a matrix with two columns, then the variogram is computed for all rows of this matrix, and these are again summed to return a single value. Alternatively, a 4-dimensional array with dimensions $(p, q, p, q)$ can be specified, which contains the weights assigned to each pair of coordinates. The isotropy pre-rank function also depends on a parameter \code{h} that can be specified by the user. This must be a single integer, denoting the lag at which the variogram differences are calculated; by default, the isotropy pre-rank function uses \code{h = 1}.
...
This can be repeated for all pre-rank functions, and for different types of errors. For example, when there is a positive bias in the forecasts, rather than a negative one: \code{mu_x = rep(0.25, d)}.

<<plot_histograms_1_gE, echo=FALSE, fig.show='hide'>>=
pr_names <- c("Multivariate", "Average rank", "Band-depth",
              "Energy score", "Mean", "Variance", "Dependence")
plot_list <- lapply(seq_along(pr_names), function(i) {
  pit_hist(rank_df[[i]], ylab = pr_names[i], ymax = 0.28, xlab = NULL, xticks = F, yticks = F)
})
plot_list[["ncol"]] <- 1
...
plot_list <- lapply(rank_df, pit_hist, ylab = "", ymax = 0.28, xlab = NULL, xticks = F, yticks = F)
plot_list[["ncol"]] <- 1
...
y <- grf(d, grid = "reg", cov.pars = c(sig2, tau), aniso.pars = c(0, 1.1), nsim = n, messages = F)$data
dim(y) <- c(p, q, n)
...
grid.arrange(plot_ifs, plot_ecc, nrow = 1)
@
...
Multivariate rank histograms can be obtained using the \pkg{MultivCalibration} functionality.
...
plot_ranks <- function(rank_df, ylabs = NULL, ymax = 0.31) {
...
plot_ifs <- plot_ranks(rank_df_ifs, ylabs = ylabs)
plot_ecc <- plot_ranks(rank_df_ecc)
plot_ss <- plot_ranks(rank_df_ss)
...
plot_evals <- function(mv_ranks_ifs, mv_ranks_ecc, mv_ranks_ss) {
  mv_ranks <- list()
...
plot_bdr <- plot_evals(rank_df_ifs$bdr, rank_df_ecc$bdr, rank_df_ss$bdr)
plot_var <- plot_evals(rank_df_ifs$var, rank_df_ecc$var, rank_df_ss$var)
plot_vgr <- plot_evals(rank_df_ifs$dep, rank_df_ecc$dep, rank_df_ss$dep)
@
